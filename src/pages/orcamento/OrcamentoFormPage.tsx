...
export const OrcamentoFormPage = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [typesResult, warrantiesResult, clientsResult] = await Promise.all([
          supabase.from('device_types').select('*').order('name').abortSignal(controller.signal),
          supabase.from('warranty_periods').select('*').order('months').abortSignal(controller.signal),
          supabase.from('clients').select('id, name, phone, email').order('created_at', { ascending: false }).abortSignal(controller.signal)
        ]);
        ...
        <div className="max-w-2xl mx-auto">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <Card className="bg-card border-border">
              ...
              <div>
                <Label className="text-xs sm:text-sm font-medium text-foreground mb-1 block">Tipo de Dispositivo</Label>
                <Select 
              ...
              <div>
                <Label className="text-xs sm:text-sm font-medium text-foreground mb-1 block">Modelo do Aparelho*</Label>
                <Input 
                  type="text" 
                  inputMode="text" 
              ...
              <div>
                <Label className="text-xs sm:text-sm font-medium text-foreground mb-1 block">Qualidade da peça</Label>
                <Input 
                  type="text" 
                  inputMode="text" 
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
              <div>
                <Label className="text-xs sm:text-sm font-medium text-foreground mb-1 block">Selecionar Cliente</Label>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {selectedClientId && selectedClientId !== 'new' 
                    ? 'Cliente selecionado. Você pode editar os dados se necessário.' 
                    : 'Insira os dados do novo cliente ou selecione um existente acima.'}
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
              <div>
                <Label className="text-xs sm:text-sm font-medium text-foreground mb-1 block">Valor à Vista (R$)*</Label>
                <Input 
                  type="number" 
                  step="0.01" 
                  inputMode="decimal" 
              ...
              {formData.enableInstallmentPrice && (
                <>
                  ...
                  <div>
                    <Label className="text-xs sm:text-sm font-medium text-foreground mb-1 block">Número de Parcelas</Label>
                    <Select 
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4 sm:pt-6">
              ...
              <Button
                ...
                className="flex-1 h-10 sm:h-11 text-sm sm:text-base"
              >
                {isSubmitting ? 'Salvando...' : (mode === 'edit' ? 'Atualizar' : 'Criar Orçamento')}
              </Button>
            </div>
          </form>
        </div>
